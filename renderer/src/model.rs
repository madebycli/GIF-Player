...
pub const HOP_HEIGHT: f64 = 60.0;
pub const HOP_DURATION: f64 = 0.65;
...
pub fn bounce_axis(position: f64, velocity: f64, dt: f64, bound: f64, item_size: f64) -> BounceAxis {
    ...
    let item_size = if item_size.is_finite() { item_size.max(0.0) } else { 0.0 };
    let extent = bound - item_size;
    if extent <= 0.0 {
        return BounceAxis { position: extent / 2.0, velocity: 0.0, oversized: true };
    }
    if velocity == 0.0 || dt == 0.0 {
        return BounceAxis { position: position.clamp(0.0, extent), velocity, oversized: false };
    }
    ...
    if phase <= extent {
        BounceAxis { position: phase, velocity, oversized: false }
    } else {
        BounceAxis { position: period - phase, velocity: -velocity, oversized: false }
    }
}
...
pub fn random_jump_delay(mean: f64) -> f64 {
    let mean = finite_clamp(mean, 0.5, 60.0, DEFAULT_JUMP_RATE);
    let u = fastrand::f64().clamp(1.0e-9, 1.0 - 1.0e-9);
    (-mean * (1.0 - u).ln()).clamp(0.7, mean * 6.0)
}
...
mod tests {
    ...
    #[test]
    fn empty_bounds_force_canvas_semantics() {
        assert!(!fully_inside(0.0, 0.0, 20.0, 20.0, 0.0, 0.0));
    }
